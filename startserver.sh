...
if [ "$(uname -m)" = "x86_64" ]; then
	./7DaysToDieServer.x86_64 -logfile /dev/stdout -quit -batchmode -nographics -dedicated $PARAMS
else
	./7DaysToDieServer.x86 -logfile /dev/stdout -quit -batchmode -nographics -dedicated $PARAMS
fi
